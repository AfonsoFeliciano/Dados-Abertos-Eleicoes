...
# MAGIC %md 
# MAGIC ## Realizando leitura e criando DM_OCUPACAO

# COMMAND ----------

df = spark.read.parquet(path_silver + file_consulta_cand).select("CD_OCUPACAO", "DS_OCUPACAO").distinct()
df.display()
df.printSchema()
...
fn_write_parquet(df, path_gold_dm, "dm_ocupacao", mode)
