...
# MAGIC %md 
# MAGIC ## Realizando leitura e criando DM_FEDERACAO

# COMMAND ----------

df = spark.read.parquet(path_silver + file_consulta_cand).select("NR_FEDERACAO", "NM_FEDERACAO", "SG_FEDERACAO", "DS_COMPOSICAO_FEDERACAO").distinct()
df.display()
df.printSchema()
...
fn_write_parquet(df, path_gold_dm, "dm_federacao", mode)