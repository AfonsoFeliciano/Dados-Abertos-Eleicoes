...
df = spark.read.parquet(path_silver + file_bem_cand).select("CD_TIPO_BEM_CANDIDATO", "DS_TIPO_BEM_CANDIDATO", "DS_BEM_CANDIDATO").distinct()
df.display()
df.printSchema()
...
fn_write_parquet(df, path_gold_dm, "dm_tipo_bem_candidato", mode)