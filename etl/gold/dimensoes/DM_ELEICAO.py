...
# MAGIC %md 
# MAGIC ## Realizando leitura e criando DM_ELEICAO

# COMMAND ----------

df = spark.read.parquet(path_silver + file_consulta_cand).select("CD_ELEICAO", "DS_ELEICAO", "CD_TIPO_ELEICAO", "NM_TIPO_ELEICAO", "TP_ABRANGENCIA").distinct()
df.display()
df.printSchema()
...
fn_write_parquet(df, path_gold_dm, "dm_eleicao", mode)