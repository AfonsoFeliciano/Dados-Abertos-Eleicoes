...
# MAGIC %md 
# MAGIC ## Realizando leitura e criando DM_ESTADO_CIVIL

# COMMAND ----------

df = spark.read.parquet(path_silver + file_consulta_cand).select("CD_ESTADO_CIVIL", "DS_ESTADO_CIVIL").distinct()
df.display()
df.printSchema()
...
fn_write_parquet(df, path_gold_dm, "dm_estado_civil", mode)