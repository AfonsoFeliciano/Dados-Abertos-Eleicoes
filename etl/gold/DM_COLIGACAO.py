...
# MAGIC %md 
# MAGIC ## Realizando leitura e criando DM_COLIGACAO

# COMMAND ----------

df = spark.read.parquet(path_silver + file_consulta_cand).select("SQ_COLIGACAO", "NM_COLIGACAO", "DS_COMPOSICAO_COLIGACAO").distinct()
df.display()
df.printSchema()
...
fn_write_parquet(df, path_gold_dm, "dm_coligacao", mode)
